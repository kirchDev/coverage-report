...
- **`release-please.yml` also publishes to npm**, in two jobs after the release-please job, matching `envprism` and `forgemap`. `publish-release` runs when release-please actually cut a release, checks out the tag and publishes `--tag latest`; `publish-prerelease` runs on every push to `dev` and on every `main` push that is not a release-please bump, stamping `<version>-dev.<run>-<sha>` onto a throwaway `dev` dist-tag. Both use `pnpm publish --provenance` with npm Trusted Publishing (OIDC) — no `NPM_TOKEN` exists here. Their build step is **`pnpm build:npm`, not `pnpm build`**: the published artifact is `lib/`, while `pnpm build` writes the committed action bundle in `dist/` that `check:dist` gates. The trusted publisher is configured on the npm package, so the first publish has to happen by hand.
