...
async function readBaseState(client, { owner, repo, branch, path }) {
...
function icon(pct, threshold) {
  if (threshold === void 0 || threshold === null) return "\u26AA";
  return pct >= threshold ? "\u{1F7E2}" : "\u{1F534}";
}
function renderCheckSummary({ totals, patch }) {
  const parts = [
...
function thresholdFailures({ totals, patch, thresholds = {} }) {
  const failures = [];
...
function parseContent(content, { format: format2, ...options } = {}) {
